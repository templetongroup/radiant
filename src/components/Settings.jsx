...
const GUIDE = [
  {
    ...
    items: [
      ...
      ['Answering a question the agent asks', 'When an agent stops to ask you something, the answers were solid blue buttons \u2014 and since each answer is usually a whole sentence, they came out as fat wrapping blocks shouting over the question itself. They are quiet rows now, with a single highlight that travels to whichever one you are on. Arrow keys move it and Enter picks, so you never have to reach for the mouse. Tony: "these selection cards are awful. ugly."'],
      ['Set how hard a model thinks', 'Radiant never asked models for a thinking level \u2014 every one ran at whatever its provider defaults to, and there was no way to see or change it. The composer now has a four-stop rail: Auto, Low, Medium, High. Auto behaves exactly as before, sending nothing, so models that do not reason are unaffected. It is remembered per chat, and if a model cannot take a level Radiant quietly runs it at the default rather than failing the turn.'],
      ...
      ['The command palette has a button', 'The palette was reachable only by \u2318K, so it did not exist unless someone told you the shortcut. There is a button for it in the composer now, and it unfolds its actions rather than just appearing.'],
