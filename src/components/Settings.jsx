...
const GUIDE = [
  {
    title: 'Chat & agents',
    items: [
      ['Closing a chat archives it now', 'The \u2715 on a chat used to delete it outright \u2014 every message and tool call gone, behind one confirm, on a button sitting next to rename. It now archives instead: the chat leaves the sidebar and collects in an Archived group at the bottom, one click from coming back. It stays searchable the whole time, so an agent looking through your past work still finds it. Deleting permanently still exists, but only from inside the archive, and it says plainly what it erases. Thanks to Justin Sail, who noticed after leaving a multi-hour review in a chat and seeing how close that \u2715 was to the paperclip.'],
      ['A board for work you hand off', 'Tasks, next to Chats in the sidebar. Write what needs doing, pick an agent or a model, and it runs as its own chat \u2014 so everything you already know about chats applies to it. The columns are Queued, Working, Needs you, Review and Done. Only the first and last are yours to drag: the middle three are set by the run itself, so a card cannot claim progress the agent did not make. A card in Working shows the agent\u2019s own checklist and the step it is on, and one lands in Needs you the moment the agent asks a question or wants permission. That column is the point of the board \u2014 until now, work waiting on you was invisible unless you happened to be looking at that chat.'],
